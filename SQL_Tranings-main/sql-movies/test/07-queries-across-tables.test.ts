...
describe("Queries Across Tables", () => {
  ...
  it(
    ...
    async done => {
      const query = `SELECT directors.full_name AS director, ROUND(SUM(movies.budget_adjusted), 2) AS total_budget FROM movies
  ...
  it(
    ...
    async done => {
      const query = `SELECT keywords.keyword, count(*) AS count FROM keywords
  ...
  it(
    ...
    async done => {
      const query = `SELECT genres.genre AS genre, COUNT(movies.id) AS five_stars_count FROM genres
  ...
  it(
    ...
    async done => {
      const query = `SELECT genres.genre AS genre, ROUND(AVG(movie_ratings.rating), 2) AS avg_rating FROM genres
      LEFT JOIN movie_genres ON genres.id = movie_genres.genre_id
      LEFT JOIN movies ON movie_genres.movie_id = movies.id
      LEFT JOIN movie_ratings ON movies.id = movie_ratings.movie_id
      GROUP BY genre
